document.querySelectorAll(".access-menu>li").forEach((item) => {
  ...
  const tl_master_piece = gsap.timeline({
    scrollTrigger: {
      trigger: ".master-piece",
      start: "top 80%",
      toggleActions: "play none none reverse",
    },
  });
  tl_master_piece.to(".master-piece img", {
    delay: 0,
    transform: "translateY(0px) skewY(0deg)",
    opacity: 1,
    duration: 1,
  ...
  tl_master_piece.from(".master-div>h1 span, .master-div>.h1 span", {
    delay: -1,
    duration: 0.5,
    position: "relative",
    top: "100px",
  ...
  tl_master_piece.from(".master-div>h4>h4", {
    delay: -0.5,
    opacity: 0,
    duration: 0.5,
    ease: "power4.out",
  ...
  tl_master_piece.from(".master-div>div", {
    delay: -1.3,
    duration: 1,
    y: 200,
    // skewY: 7,
  ...
  tl_tropo.from(".gourmet-item>h4 h4", {
    delay: -0.2,
    duration: 1,
    opacity: 0,
    stagger: 0.1,
  ...
  tl_tropo.from(".gourmet-item>h1 span, .gourmet-item>.h1 span", {
    delay: -1.5,
    duration: 0.5,
    position: "relative",
    top: "100px",
  ...
  tl_tropo.from(".gourmet-item>div", {
    delay: -0.8,
    y: 300,
    duration: 1,
    opacity: 0,
  ...
  if ($(".social")) {
    ...
    tl_social.from(".social>h1 span, .social>.h1 span", {
      // delay: -0.5,
      duration: 1,
      position: "relative",
      top: "100px",
    ...
    // tl_social.from(".social-posts>div a", {
    //   delay: 1,
    //   y: 400,
    //   // skewY: 10,
    //   duration: 1,
  ...
  tl_truck.from(".deals>div h1 span, .deals>div .h1 span", {
    // delay: -0.2,
    duration: 0.5,
    position: "relative",
    top: "100px",
  ...
  if ($(".bakery")) {
    ...
    //   // skewY: 7,
    // });
    // tl_bakery.from(".bakery>div>h4 span", {
    //   delay: -0.3,
    //   duration: 0.5,
    //   position: "relative",
    //   top: "50px",
  ...
  // $('.gal-dis').on('touchmove', function(event) {
  //     if (!startX) return; // Touch start position not recorded
  
  //     const currentX = event.originalEvent.touches[0].clientX;
  //     const deltaX = currentX - startX;
  //     var current_n = $(".gal-opt img.active, .gal-opt video.active .gal-opt .embed_video.active");
  //     console.log(current_n)
  //     var next_n;
  //     if (Math.abs(deltaX) > 40) {
  //         if (deltaX > 0) {
  //             next_n = current_n.prev();
  //             if (!next_n.length) {
  //               // If there is no previous element, wrap around to the last element
  //               next_n = $(".gal-opt img, .gal-opt video, .embed_video.active").last().addClass('active');
  //             }
  //         } else {
  //           next_n = current_n.next();
  //           if (!next_n.length) {
  //             // If there is no next element, wrap around to the first element
  //             next_n = $(".gal-opt img, .gal-opt video, .embed_video.active").first().addClass('active');
  
  //           }
  //         }
  //         current_n.removeClass("active");
  //         next_n.addClass("active");
  //         var title_imggg = next_n[0].attributes.title.value
  
  //         updateGalleryDisplay(next_n, title_imggg);
  //         startX = null;
  //     }
  ...
  var touchStartX;
  var threshold = 50; // Adjust this value based on your sensitivity
  ...
  function updateGalleryDisplay(item, title_imgg, thiss) {
    ...
    if (item.prop("tagName").toLowerCase() === "div") {
      console.log($(thiss.children).attr("src"));
      $(".gal-dis").children().hide();
      $(".gal-dis .embed_video").show();
  ...
  $(".gal-opt img, .gal-opt video, .gal-opt .embed_video").on(
    ...
    function () {
      ...
      var thiss = this;
      var title_img;
      // if(item[0].children[0].tagName === 'IFRAME'){
      //    item = $(this.children)
      // } else {
      item = $(this);
  ...
  function handleKeyDown(e) {
    if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
      var current = $(".gal-opt img.active, .gal-opt video.active");
      var next;
      ...
      if (e.key === "ArrowLeft") {
        ...
        if (!next.length) {
          // If there is no previous element, wrap around to the last element
          next = $(".gal-opt img, .gal-opt video").last().addClass("active");
        }
      } else {
        next = current.next();
        if (!next.length) {
          // If there is no next element, wrap around to the first element
          next = $(".gal-opt img, .gal-opt video").first().addClass("active");
        }
      }
  
      current.removeClass("active");
      next.addClass("active");
      var title_imggg = next[0].attributes.title.value;
  
      updateGalleryDisplay(next, title_imggg);
    }
  }
  
  $(document.body).on("click", function (e) {
    if (!$(e.target).closest(".gal-opt").length) {
