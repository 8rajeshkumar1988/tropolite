...
(function ($, window, document, undefined) {
  var Instagram = {
    ...
    displayPhotos: function (results) {
      ...
      max = this.options.max >= results.data.length
          ? results.data.length
          : this.options.max;
      ...
      for (var i = 0; i < 9; i++) {
        ...
        if (
          ...
          imageCaption = hasCaption
            ...
            // if(results.data[i].caption.indexOf('#tasty') !== -1){
            //   console.log(results.data[i].caption);
            // }
            // console.log(results.data[i].caption);
            // $source = $("<source>", {
            ...
            $image = $(`<img src='${results.data[i].media_url}'  loading='lazy' >`)
            $image.attr('alt', `${results.data[i].caption.slice(0, 200)}`)
          $element = $("<a>", {
            href: results.data[i].permalink,
            target: "_blank",
            title: 'tropolite instagram feed',
            // style:
            //   "background:url(" +
          ...
          // $igback = $('<div>', {class: 'ig-back'})

          $maindiv = $('<div>', {class: 'rat'}).append($element)
          // $maindiv.append($igback)
          // Add item
          imageGroup.push($maindiv);
