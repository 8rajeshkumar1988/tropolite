...
from app.models import Statics, CsrReport, CsrHighlight, Bakery, Category, Faq, CsrCategory
from app.utils import category_list, get_page_data, get_random_recipes, get_breadcrumb_json
...
def artisan(request):
    ...
    products = Product.objects.all().order_by(
        "sequence_number").filter(is_active=True, tags__slug='prime').order_by('?')
    context['products'] = products
...
def bakery_locator(request):
    ...
    pincodes = Bakery.objects.all().order_by(
        "name").filter(is_active=True).values_list('pin_code', flat=True).distinct()
...
def csrreport_list(request):
    ...
    csrreports = CsrReport.objects.all().order_by(
        "-report_date").filter(is_active=True)
...
def location_list(request):
    ...
    if pin_code is not None and pin_code and pin_code != 'None' and pin_code != 'ALL':
        ...
        if not queryset.exists():
            ...
            queryset = Bakery.objects.all().order_by(
                "name").filter(is_active=True, state__iexact=state)
...
def food_dairy_technology(request):
    ...
    context['itemListElement'] = json.dumps(get_breadcrumb_json(
        breadcrumbs, request, "food-dairy-technology"))
    context['leadform'] = LeadForm()
...
def factory(request):
    ...
    context['page_url'] = request.build_absolute_uri()
    context['itemListElement'] = json.dumps(get_breadcrumb_json(
        breadcrumbs, request, "manufacturing-capabilities"))
    context['leadform'] = LeadForm()
...
def department_list():
    array = []
    queryset = JobDepartment.objects.all().order_by(
        'sequence_number').filter(is_active=True)
    for query in queryset:
        rec = {}
        rec['id'] = query.id
...
def department_job_list(department):
    ...
    queryset = Job.objects.all().order_by(
        'id').filter(is_active=True, job_department_id=department)
    if queryset:
        for query in queryset:
            rec = {}
            rec['id'] = query.id
...
def export_csv_and_send_email(request):
    ...
    csv_writer.writerow(['Name', 'Mobile', 'Email', 'Query', 'City', 'Zip Code', 'State',
                        'Category', 'Sub Category', 'Product'])  # Replace with your actual field names
    ...
    for record in records:
        ...
        csv_writer.writerow([record.name, record.mobile, record.email, record.description, record.city,
                            record.zip_code, record.state, category, subcategory, product])
        ...
        if mobile:
            if validate_mobile_number(mobile):
                ...
                if lead_form.is_valid():
                    led_count = Lead.objects.filter(name=name,mobile=mobile,email=email,description=description).count()
                    if led_count>0:
                        response_data['status'] = True
                        response_data['class'] = 'error'
                    ...
                    response_data['message'] = "Our customer support team will get back to you promptly."
                    response_data['errors'] = ""
                else:
                    response_data = {}
                    response_data['error'] = True
                ...
                response_data['error'] = True
                response_data['message'] = "The Phone number " + \
                    str(mobile)+" is not valid."
                response_data['class'] = "error"
                response_data['errors'] = ""
        else:
    ...
    with get_connection(
        ...
        if applicant.resume:
            if settings.SERVER_TYPE == "P":
                msg.attach_file("/var/www/tropolite/media/" +
                                str(applicant.resume))
            else:
...
def csr(request):
    ...
    csrhighlights = CsrHighlight.objects.all().order_by(
        "sequence_number").filter(is_active=True)[:20]
    context['csrhighlights'] = csrhighlights

    csrreports = CsrReport.objects.all().order_by(
        "-report_date").filter(is_active=True)[:3]
    context['csrreports'] = csrreports
    context['itemListElement'] = json.dumps(
        get_breadcrumb_json(breadcrumbs, request, "csr"))
    context['leadform'] = LeadForm()

    csr_category_list = CsrCategory.objects.all().order_by(
        "sequence_number").filter(is_active=True)
...
def event_list(request):
    ...
    eventLists = Event.objects.all().order_by(
        "created_at").filter(is_active=True)
...
def microbial_biotechnology(request):
    ...
    context['page_url'] = request.build_absolute_uri()
    context['itemListElement'] = json.dumps(get_breadcrumb_json(
        breadcrumbs, request, "microbial-biotechnology"))
    context['leadform'] = LeadForm()
