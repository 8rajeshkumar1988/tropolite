...
class BlogAdmin(admin.ModelAdmin):
    list_display = ('full_heading','is_active', 'created_at','canonical_url','preview_link')
    ...
    def full_heading(self, obj):
        if obj.sub_heading:
          ...
          return f"{obj.heading}"  
    full_heading.short_description = 'Heading'
    def preview_link(self, obj):
        url = reverse('blog_detail', args=[obj.slug])  # Replace 'recipe_detail' with your actual URL name
    ...
    def export_selected(self, request, queryset):
        ...
        writer.writerow([field.name for field in Blog._meta.fields])  # Header row

        for obj in queryset:
            writer.writerow([getattr(obj, field.name) for field in Blog._meta.fields])
